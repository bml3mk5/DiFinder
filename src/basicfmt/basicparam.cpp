...
void DiskBasicParam::SetBasicParam(const DiskBasicParam &src)
{
	...
	based_sector_size = src.based_sector_size;
	sectors_per_group = src.sectors_per_group;
	number_of_sectors = src.number_of_sectors;
	managed_sector_number = src.managed_sector_number;
...
void DiskBasicParam::CalcDirStartEndSector(int sector_size)
{
	if (dir_start_sector < 0) {
		dir_start_sector = reserved_sectors + number_of_fats * sectors_per_fat;
	}
	if (dir_end_sector < 0) {
...
DiskBasicCategory::DiskBasicCategory(const wxString &n_type_name, const wxString &n_description)
...
//
DiskBasicTemplates::DiskBasicTemplates() : TemplatesBase()
{
}
/// XMLファイル読み込み
	...
	while(item && valid) {
		if (item->GetName() == "DiskBasicType") {
			...
			while (itemnode) {
				...
				} else if (name == "BasedSectorSize") {
					p.SetBasedSectorSize(Utils::ToInt(str));
				} else if (name == "SectorsPerGroup") {
					p.SetSectorsPerGroup(Utils::ToInt(str));
				} else if (name == "NumberOfSectors") {
					p.SetNumberOfSectorsOnBasic(Utils::ToInt(str));
				} else if (name == "ManagedSectorNumber") {
					p.SetManagedSectorNumber(Utils::ToInt(str));
...
//					p.SideReversed(Utils::ToBool(str));
					...
					MyAttributes attrs;
					LoadMyAttributesInTypes(itemnode, locale_name, errmsgs, attrs);
...
/// @param[out] attrs       値
/// @return true
bool DiskBasicTemplates::LoadMyAttributesInTypes(const wxXmlNode *node, const wxString &locale_name, wxString &errmsgs, MyAttributes &attrs)
{
	wxXmlNode *citemnode = node->GetChildren();
	while(citemnode) {
...
/// @param[out] attrs       値
/// @return true
bool DiskBasicTemplates::LoadMyAttribute(const wxXmlNode *node, const wxString &locale_name, int type, MyAttributes &attrs)
{
	wxString desc, desc_locale;
...
/// @return 一致したパラメータ
const DiskBasicParam *DiskBasicTemplates::FindType(const wxString &n_category, const BasicParamNames &n_basic_types) const
{
...
/// @return パラメータリストの数
size_t DiskBasicTemplates::FindParams(const BasicParamNames &n_type_names, DiskBasicParamPtrs &params) const
{
...
const DiskBasicCategory *DiskBasicTemplates::FindCategory(const wxString &n_category) const
{
	...
	for(size_t n=0; n<categories.Count(); n++) {
		...
		if (item->GetTypeName() == n_category) {
			match = item;
			break;
		}
	}
