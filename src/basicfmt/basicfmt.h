...
//	/// ディレクトリアイテムの作成 使用後はdeleteすること
//	DiskBasicDirItem *CreateDirItem(int n_block_num, int n_position, wxUint8 *n_data);
	/// ディレクトリアイテムの作成 使用後はdeleteすること
	DiskBasicDirItem *CreateDirItem(int n_block_num, int n_position);
	/// ディレクトリのアイテムを取得
	...
	//@{
	/// グループ番号からセクタを返す
	DiskImageSector	*GetSectorFromGroup(wxUint32 group_num);
//	/// グループ番号からセクタを返す
//	DiskImageSector	*GetSectorFromGroup(wxUint32 group_num, int *div_num, int *div_nums);
	...
	bool			CalcStartNumFromGroupNum(wxUint32 group_num, int &start_block, int *sector_pos = NULL);
//	/// グループ番号から開始セクタ番号(パーティション内とディスク全体)を計算(グループ計算用)
//	bool			CalcStartNumFromGroupNum(wxUint32 group_num, int &start_block, int *sector_pos, int *div_num, int *div_nums);
	//@}
	/// @name ユーティリティ
	//@{
	/// パーティション内のセクタ位置からセクタ返す
	DiskImageSector	*GetSector(int block_num);
//	/// セクタ位置(トラック0,サイド0,セクタ1を0とした通し番号)からセクタを返す
//	DiskImageSector	*GetSector(int block_num, int *div_num, int *div_nums);

	/// キャッシュを更新する
	void			RefreshCache(int block_num);
