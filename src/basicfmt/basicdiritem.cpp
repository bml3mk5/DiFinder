...
/// ディレクトリアイテムを作成 DATAはセクタをその都度アクセス
/// @param [in]  basic        DISK BASIC
/// @param [in]  n_block_num  パーティション内のセクタ番号
/// @param [in]  n_position   セクタ内の位置
DiskBasicDirItem::DiskBasicDirItem(DiskBasic *basic, int n_block_num, int n_position)
{
	this->basic = basic;
	this->type = basic->GetType();
...
DiskBasicDirItem::DiskBasicDirItem(DiskBasic *basic, int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next, bool &n_unuse)
{
	this->basic = basic;
	this->type = basic->GetType();
...
/// @param [in] n_next       次のセクタ
void DiskBasicDirItem::SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, wxUint8 *n_data, const int *n_next)
{
	m_num = n_num;
	m_position = n_position;
	m_block_num = n_block_num;
...
/// @param [in] n_position   セクタ内のディレクトリエントリの位置
/// @param [in] n_next       次のセクタ
void DiskBasicDirItem::SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next)
{
	m_num = n_num;
//	m_position = n_position;
//	m_block_num = n_block_num;
...
void DiskBasicDirItem::RefreshCache() const
{
//	if (m_block_num >= 0) {
//		basic->RefreshCache(m_block_num);
//	}
}
...
void DiskBasicDirItem::SetCommonDataInAttrDialog(KeyValArray &vals) const
{
	...
	vals.Add(wxT("position"), (wxUint32)GetPosition());
...
bool DirItemSectorBoundary::Set(DiskBasic *basic, DiskImageSector *sector, int position, directory_t *item_data, size_t item_size, const int *next)
{
...
//			DiskBasicType *type = basic->GetType();
			int n_block_num = sector->GetNumber() + 1;
			DiskImageSector *nsector = basic->GetSector(n_block_num);
			if (nsector) {
				sptr = nsector->GetSectorBuffer();
				ssize = nsector->GetSectorSize();
				spos = basic->GetDirStartPosOnSector();
				spos += (n_block_num % basic->GetSectorsPerGroup()) == 0 ? basic->GetDirStartPosOnGroup() : 0;
