...
private:
	...
	DiskImageDisk *p_disk;	///< ディスク
	int   m_position;	///< セクタ内の位置（バイト）
	int	  m_block_num;	///< ディレクトリのあるセクタ番号
public:
	DiskBasicDirData();
	~DiskBasicDirData();
	/// @brief 削除
	...
	void Alloc();
	/// @brief ポインタ割り当て
	void Attach(DiskImageDisk *disk, int block_num, int position);
	/// @brief コピー
	bool Copy(const void *data, size_t len = 0xfffffff, size_t start = 0);
	/// @brief 埋める
	bool Fill(int ch, size_t len = 0xfffffff, size_t start = 0);
//	/// @brief 反転
//	void Invert(size_t len = 0xfffffff, size_t start = 0);
	...
	/// @brief セクタ番号を返す
	int	GetBlockNumber() const { return m_block_num; }
...
/// @param[in] data ポインタ
template <class TYPE>
void DiskBasicDirData<TYPE>::Attach(DiskImageDisk *disk, int block_num, int position)
{
	Delete();
	p_disk = disk;
...
template <class TYPE>
bool DiskBasicDirData<TYPE>::Copy(const void *data, size_t len, size_t start)
{
	if (!p_disk && !m_data) return false;

	wxUint8 *src = (wxUint8 *)data;
	if (len > sizeof(TYPE)) len = sizeof(TYPE);
	if (p_disk) {
		DiskImageSector *sector = p_disk->GetSector(m_block_num);
		sector->Copy(src + start, (int)len, m_position + (int)start);
	} else if (m_data) {
...
/// @param[in] start  コピー開始位置
template <class TYPE>
bool DiskBasicDirData<TYPE>::Fill(int ch, size_t len, size_t start)
{
	if (!p_disk && !m_data) return false;
	...
	if (p_disk) {
		DiskImageSector *sector = p_disk->GetSector(m_block_num);
		sector->Fill(ch, (int)len, m_position + (int)start);
	} else if (m_data) {
		wxUint8 *dst = (wxUint8 *)m_data;
...
/// @brief データを返す
template <class TYPE>
TYPE *DiskBasicDirData<TYPE>::Data() const
{
	if (!p_disk && !m_data) return NULL;

	if (p_disk) {
		DiskImageSector *sector = p_disk->GetSector(m_block_num);
		return (TYPE *)sector->GetSectorBuffer(m_position);
	} else if (m_data) {
		return m_data;
	}
	return NULL;
...
/// @brief データが有効か
template <class TYPE>
bool DiskBasicDirData<TYPE>::IsValid() const
{
	return (p_disk || m_data); 
}
...
public:
	...
	/// @brief ディレクトリアイテムを作成
	DiskBasicDirItem(DiskBasic *basic, int n_block_num, int n_position);
	/// @brief ディレクトリアイテムを作成
	DiskBasicDirItem(DiskBasic *basic, int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next, bool &n_unuse);
...
//	virtual void	SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, wxUint8 *n_data, const int *n_next = NULL);
	...
	/// @brief チェイン用のセクタをセット(機種依存)
	virtual void	SetChainSector(int block_num, wxUint32 group_num, wxUint8 *data, const DiskBasicDirItem *pitem = NULL) {}
	/// @brief チェイン用のセクタをセット(機種依存)
...
public:
	...
	void Clear();
	/// @brief アイテムの位置情報をセット
	bool Set(DiskBasic *basic, DiskImageSector *sector, int position, directory_t *item_data, size_t item_size, const int *next);
	/// @brief コピー
