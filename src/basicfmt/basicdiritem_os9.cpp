...
#endif
/// セクタデータを取得しfdとする
directory_os9_fd_t *DiskBasicDirItemOS9FD::GetFDFromSector() const
{
	directory_os9_fd_t *fd = NULL;
	if (m_block_num >= 0) {
		DiskImageSector *sector = basic->GetSector(m_block_num);
		if (sector) fd = (directory_os9_fd_t *)sector->GetSectorBuffer();
	}
	return fd;
}
/// ポインタをセット
void DiskBasicDirItemOS9FD::Set(DiskBasic *n_basic, int n_block_num, wxUint32 n_mylsn)
{
	basic = n_basic;
	...
	: DiskBasicDirItem(basic, n_num, n_gitem, n_block_num, n_position, n_next, n_unuse)
{
	...
	// FDセクタへのポインタをセット
	if (IsUsed()) {
		wxUint32 lsn = GET_OS9_LSN(m_data.Data()->DE_LSN);
		if (lsn != 0) {
...
void DiskBasicDirItemOS9::SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next)
{
...
void DiskBasicDirItemOS9::GetUnitGroups(int fileunit_num, DiskBasicGroups &group_items)
{
	...
	group_items.SetSizePerGroup(basic->GetDisk()->GetFile()->GetSectorSize());
}
...
/// チェイン用のセクタをセット
/// @param [in] block_num  パーティション内のセクタ番号
/// @param [in] lsn        セクタのLSN(グループ番号)
/// @param [in] data       セクタ内のバッファ
/// @param [in] pitem      コピー元のアイテム
void DiskBasicDirItemOS9::SetChainSector(int block_num, wxUint32 lsn, wxUint8 *data, const DiskBasicDirItem *pitem)
{
	fd.Set(basic, block_num, lsn);
	fd.Clear();
...
{
	m_data.Fill(basic->GetDeleteCode(), GetDataSize(), false);
}
