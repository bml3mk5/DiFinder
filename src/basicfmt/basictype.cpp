...
/// @param [in] block_num  論理セクタ位置(トラック0,サイド0,セクタ1を0とした通し番号)
/// @param [out] track_num  トラック番号(0 .. )
/// @param [out] sector_num 論理セクタ番号(0 .. )
/// @param [out] num_of_sectors セクタ数
void DiskBasicSectorPosTrans::GetNumFromSectorPos(int block_num, int &track_num, int &sector_num, int &num_of_sectors) const
{
	track_num = 0;
	...
	for(size_t i=0; i<Count(); i++) {
		SectorsPerTrack *item = &Item(i);
		if (block_num < item->GetTotalSectors()) {
			num_of_sectors = item->GetNumOfSectors();
			track_num += (block_num / num_of_sectors);
			sector_num = block_num % num_of_sectors;
			break;
		}
		block_num -= item->GetTotalSectors();
		track_num += item->GetNumOfTracks();
	}
}
...
int DiskBasicSectorPosTrans::GetSectorPosFromNum(int track_num, int sector_num, int &num_of_sectors) const
{
	...
	for(size_t i=0; i<Count(); i++) {
		...
		if (track_num < item->GetNumOfTracks()) {
			num_of_sectors = item->GetNumOfSectors();
			block_num += track_num * num_of_sectors + sector_num;
			break;
		}
		track_num -= item->GetNumOfTracks();
		block_num += item->GetTotalSectors();
...
/// 次のグループ番号を得る
/// @param [in] num        グループ番号(0...)
/// @param [in] block_num セクタ位置
/// @return 次のグループ番号 / INVALID_GROUP_NUMBER 空きなし
wxUint32 DiskBasicType::GetNextGroupNumber(wxUint32 num, int block_num)
{
	return 0;
}
...
//	int secs_per_grp = basic->GetSectorsPerGroup();
	...
	int sed = 1; // secs_per_trk * sides / secs_per_grp;
...
/// システムグループ番号を返す
wxUint32 DiskBasicType::GetGroupSystemCode() const
{
	return basic->GetGroupSystemCode();
}

/// FATの開始位置を得る（ダイアログ用）
/// @param [out] block_num  パーティション内のセクタ番号
/// @param [out] sector_pos ディスクイメージ全体のセクタ位置
void DiskBasicType::GetStartNumOnFat(int &block_num, int &sector_pos) const
{
	block_num = -1;
	sector_pos = -1;
	int pos = basic->GetFatStartSector();
	DiskImageSector *sector = basic->GetSector(pos);
	if (sector) {
		block_num = pos;
...
/// @param [out] block_num  パーティション内のセクタ番号
/// @param [out] sector_pos ディスクイメージ全体のセクタ位置
void DiskBasicType::GetEndNumOnFat(int &block_num, int &sector_pos) const
{
	block_num = -1;
	sector_pos = -1;
	int sta = basic->GetFatStartSector();
	int pos = sta + basic->GetSectorsPerFat() * basic->GetNumberOfFats() - 1;
	DiskImageSector *sector = basic->GetSector(pos);
	if (sector) {
		block_num = pos;
		sector_pos = sector->GetNumber();
...
//		int div_num = 0;
//		int div_nums = 1;
//		DiskImageSector *sector = basic->GetSector(sec_pos, &div_num, &div_nums);
		DiskImageSector *sector = basic->GetSector(sec_pos);
		if (!sector) continue;
//		group_items.Add(sec_pos, 0, sec_pos, sec_pos, div_num, div_nums);
//		dir_size += (sector->GetSectorSize() / div_nums);
		group_items.Add(sec_pos, 0, sec_pos, sec_pos);
...
//		int div_nums = gitem->div_nums;	// 分割数
		...
		for(int block_num = gitem->GetSectorStart(); block_num <= gitem->GetSectorEnd() && finish >= -1; block_num++) {
...
//			int size = sector->GetSectorSize() / div_nums;
			...
			int next_sector = (block_num < gitem->GetSectorEnd() ? block_num + 1 : (next_gitem ? next_gitem->GetSectorStart() : -1));
...
//				buffer += basic->GetDirStartPosOnGroup();
			...
			if (idx == 0 && block_num == gitem->GetSectorStart()) {
...
//				nitem->SetDataPtr(index_number, gitem, block_num, pos, buffer, &next_sector);
				nitem->SetDataPtr(index_number, gitem, block_num, pos, &next_sector);
...
//		int div_nums = gitem->div_nums;	// 分割数
		...
		for(int block_num = gitem->GetSectorStart(); block_num <= gitem->GetSectorEnd() && valid && !last && finish >= -1; block_num++) {
			DiskImageSector *sector = basic->GetSector(block_num);
			if (!sector) {
				valid = false;
				break;
...
//				DiskBasicDirItem *nitem = dir->NewItem(index_number, gitem, block_num, pos, buffer, &next_sector, unuse);
				DiskBasicDirItem *nitem = dir->NewItem(index_number, gitem, block_num, pos, &next_sector, unuse);
...
//	DiskBasicDirItem *newitem = basic->CreateDirItem(0, 0, NULL);
	DiskBasicDirItem *newitem = basic->CreateDirItem(0, 0);
...
//		int div_nums = gitem->div_nums;	// 分割数
		...
		for(int block_num = gitem->GetSectorStart(); block_num <= gitem->GetSectorEnd() && finish >= -1 && rc >= 0; block_num++) {
			DiskImageSector *sector = basic->GetSector(block_num);
			if (!sector) {
				// セクタがない！
				errinfo.SetError(DiskBasicError::ERRV2_NO_SECTOR, grp_num, block_num);
				rc = -2;
				break;
			}
//			wxUint8 *buffer = sector->GetSectorBuffer();
...
//			buffer += basic->GetDirStartPosOnSector();
			...
			while(pos < size) {
				...
				// ディスク内に書き込む
//				newitem->SetDataPtr(index_number, gitem, block_num, pos, buffer, &next_sector);
				newitem->SetDataPtr(index_number, gitem, block_num, pos, &next_sector);
				// 初期値を入れる
				newitem->InitialData();
...
/// @param [out] block_num  パーティション内のセクタ番号
/// @param [out] sector_pos ディスクイメージ全体のセクタ位置
void DiskBasicType::GetStartNumOnRootDirectory(int &block_num, int &sector_pos) const
{
	block_num = -1;
	sector_pos = -1;
	int pos = basic->GetDirStartSector();
	DiskImageSector *sector = basic->GetSector(pos);
	if (sector) {
		block_num = pos;
		sector_pos = sector->GetNumber();
...
/// @param [out] block_num  パーティション内のセクタ番号
/// @param [out] sector_pos ディスクイメージ全体のセクタ位置
void DiskBasicType::GetEndNumOnRootDirectory(int &block_num, int &sector_pos) const
{
	block_num = -1;
	sector_pos = -1;
	int pos = basic->GetDirEndSector();
	DiskImageSector *sector = basic->GetSector(pos);
	if (sector) {
		block_num = pos;
		sector_pos = sector->GetNumber();
...
//	int groups = 0;
	...
	int bytes_per_group = basic->GetSectorsPerGroup() * basic->GetDisk()->GetSectorSize();
...
//		myLog.SetDebug("  group_num:0x%03x next:0x%03x", group_num, next_group_num);
		...
		basic->GetNumsFromGroup(group_num, next_group_num, basic->GetDisk()->GetSectorSize(), sizeremain, group_items);
	...
	if (rc >= 0) {
		if (flags == ALLOCATE_GROUPS_APPEND) {
			...
			if (group_items.Count() > 0) {
				rc = ChainGroups(item->GetStartGroup(0), group_items.Item(0).GetGroup());
			}
		}
	} else {
		// グループを削除
...
#if 0
/// パーティション内のセクタ位置から分割番号を得る
/// @param [in] block_num     パーティション内のセクタ位置
/// @param [out] div_num      分割番号
/// @param [out] div_nums     分割数
void DiskBasicType::GetNumFromSectorPos(int block_num, int *div_num, int *div_nums)
{
...
/// @param [in] sector セクタ
void DiskBasicType::FillSector(DiskImageSector *sector)
{
...
void DiskBasicType::DeleteGroups(const DiskBasicGroups &group_items)
{
	for(size_t gidx=0; gidx<group_items.Count(); gidx++) {
		// FATエントリを削除
		DeleteGroupNumber(group_items.Item(gidx).GetGroup());
	}
}
