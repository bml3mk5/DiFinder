...
//	BitMLBuffer(wxUint8 *buffer, size_t size);
	...
	/// @brief サイズ(ビット数)を返す
	int		 GetBitSize() const { return m_size << 3; }
	/// @brief バッファサイズを返す
...
//	void AddBuffer(wxUint8 *buffer, size_t size);
	/// @brief ポインタをセット
	void AddBuffer(DiskImageDisk *disk, int block_num, int start, int size);
...
public:
	...
	/// @brief 有効なバッファ数をセット
	void	SetValidCount(size_t val) { m_valid_count = val; }
	/// @brief 有効なバッファ数を返す
	size_t	GetValidCount() const { return m_valid_count; }
...
private:
	DiskBasic			*p_basic;
	DiskBasicType		*p_type;
	int m_count;		///< FATの数
	int m_vcount;		///< 使用しているFATの数
	int m_size;			///< FATサイズ(セクタ数)
	int m_start;		///< 開始セクタ番号
	int m_start_pos;	///< 開始位置

	DiskBasicFatArea m_bufs;
...
public:
	...
	/// @brief FAT領域を返す
	DiskBasicFatArea	 *GetDiskBasicFatArea() { return &m_bufs; }
	/// @brief FATバッファを返す
