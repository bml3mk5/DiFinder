...
/// @param[in] n_end   グループ内の終了セクタ番号
/// @param[in] n_user  機種依存データ
DiskBasicGroupItem::DiskBasicGroupItem(wxUint32 n_group, wxUint32 n_next, int n_start, int n_end, DiskBasicGroupUserData *n_user)
{
	this->Set(n_group, n_next, n_start, n_end, n_user);
}
/// @param[in] n_group グループ番号
/// @param[in] n_next  次のグループ番号（任意）
/// @param[in] n_start グループ内の開始セクタ番号
/// @param[in] n_user  機種依存データ
DiskBasicGroupItem::DiskBasicGroupItem(wxUint32 n_group, wxUint32 n_next, int n_start, DiskBasicGroupUserData *n_user)
{
	this->Set(n_group, n_next, n_start, n_user);
}
/// デストラクタ
DiskBasicGroupItem::~DiskBasicGroupItem()
{
...
/// @param[in] n_end   グループ内の終了セクタ番号
/// @param[in] n_user  機種依存データ
void DiskBasicGroupItem::Set(wxUint32 n_group, wxUint32 n_next, int n_start, int n_end, DiskBasicGroupUserData *n_user)
{
	m_group = n_group;
	m_next = n_next;
	m_sector_start = n_start;
...
/// @param[in] n_start グループ内の開始セクタ番号
/// @param[in] n_user  機種依存データ
void DiskBasicGroupItem::Set(wxUint32 n_group, wxUint32 n_next, int n_start, DiskBasicGroupUserData *n_user)
{
	m_group = n_group;
	m_next = n_next;
	m_sector_start = n_start;
...
{
	return ((*item1)->m_group == (*item2)->m_group ? 0 : ((*item1)->m_group > (*item2)->m_group ? 1 : -1));
}
