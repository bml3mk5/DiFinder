...
public:
	...
	/// @brief 次のグループ番号を得る
	virtual wxUint32 GetNextGroupNumber(wxUint32 num, int block_num);
	/// @brief 空きFAT位置を返す
	...
	virtual void	GetStartNumOnFat(int &block_num, int &sector_pos) const;
	/// @brief FATの終了位置を得る（ダイアログ用）
	virtual void	GetEndNumOnFat(int &block_num, int &sector_pos) const;
	...
	void			GetStartNumOnRootDirectory(int &block_num, int &sector_pos) const;
	/// @brief ルートディレクトリの終了位置を得る
	void			GetEndNumOnRootDirectory(int &block_num, int &sector_pos) const;
	//@}
	...
	/// @brief グループ番号からパーティション内の開始セクタ番号を得る
	virtual int		GetStartSectorFromGroup(wxUint32 group_num);
	/// @brief グループ番号からパーティション内の最終セクタ番号を得る
...
//	/// @brief パーティション内のセクタ位置から分割番号を得る
//	virtual void	GetNumFromSectorPos(int block_num, int *div_num, int *div_nums);
	//@}
	...
	virtual void	FillSector(DiskImageSector *sector);
