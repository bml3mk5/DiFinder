...
DiskBasicDirItemHFS::DiskBasicDirItemHFS(DiskBasic *basic, int n_block_num, int n_position)
	: DiskBasicDirItem(basic, n_block_num, n_position)
{
#if 0
	if (!n_block_num) return;

	m_key.Attach(basic->GetDisk(), n_block_num, n_position);
	...
	if (rpos <= 37) {
		...
		m_data.Attach(basic->GetDisk(), n_block_num, n_position + rpos);
	}
#endif
}
DiskBasicDirItemHFS::DiskBasicDirItemHFS(DiskBasic *basic, int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next, bool &n_unuse)
...
void DiskBasicDirItemHFS::SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next)
{
...
void DiskBasicDirItemHFS::GetUnitGroupsDir(int fileunit_num, const hfs_cat_dir_rec_t *dir, DiskBasicGroups &group_items)
{
	...
	for(int sec = start_sector; sec < end_sector; sec++) {
		DiskImageSector *sector = basic->GetSector(sec);
		if (!sector) {
			break;
		}
	...
	group_items.SetSizePerGroup(basic->GetSectorsPerGroup() * basic->GetDisk()->GetSectorSize());
}
...
void DiskBasicDirItemHFS::GetGroupsFromExtDataRec(const hfs_ext_data_rec_t *exts, DiskBasicGroups &group_items)
{
	...
	for(int i=0; i<3; i++) {
		...
		for(wxUint32 grp = start_group; grp < end_group; grp++) {
			...
			int block_num = type->GetStartSectorFromGroup(grp);

			group_items.Add(grp, 0, block_num, block_num + sec_per_grp - 1);
...
int DiskBasicDirItemHFS::RecalcFileSize(DiskBasicGroups &group_items, int occupied_size)
{
	...
	DiskImageSector *sector = basic->GetSector(litem->GetSectorStart());
...
int DiskBasicDirItemHFS::RecalcFileSizeOnSave(wxInputStream *istream, int file_size)
{
	if (NeedCheckEofCode()) {
		...
		if ((file_size % basic->GetDisk()->GetSectorSize()) != 0) {
...
{
	int limit_size = 1 * basic->GetDisk()->GetSectorSize() - 1;
