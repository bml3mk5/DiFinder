...
private:
	...
	directory_os9_fd_t *GetFDFromSector() const;
...
#endif
	/// @brief ポインタをセット
	void Set(DiskBasic *n_basic, int n_block_num, wxUint32 n_mylsn);
	/// @brief FDのメモリ確保
	void Alloc();
	/// @brief FDをクリア
	void Clear();
...
//	DiskBasicDirItemOS9(DiskBasic *basic, int n_block_num, int n_position, wxUint8 *n_data);
//	DiskBasicDirItemOS9(DiskBasic *basic, int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, wxUint8 *n_data, const int *n_next, bool &n_unuse);
...
//	virtual void	SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, wxUint8 *n_data, const int *n_next = NULL);;
	...
	/// @brief チェイン用のセクタをセット
	virtual void	SetChainSector(int block_num, wxUint32 lsn, wxUint8 *data, const DiskBasicDirItem *pitem = NULL);
