...
#if 0
DiskBasicDirItemMSDOS::DiskBasicDirItemMSDOS(DiskBasic *basic, int n_block_num, int n_position, wxUint8 *n_data)
	: DiskBasicDirItem(basic, n_block_num, n_position, n_data)
{
	m_data.Attach(n_data);
}
DiskBasicDirItemMSDOS::DiskBasicDirItemMSDOS(DiskBasic *basic, int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, wxUint8 *n_data, const int *n_next, bool &n_unuse)
...
void DiskBasicDirItemMSDOS::SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, wxUint8 *n_data, const int *n_next)
{
...
#endif
DiskBasicDirItemMSDOS::DiskBasicDirItemMSDOS(DiskBasic *basic, int n_block_num, int n_position)
	: DiskBasicDirItem(basic, n_block_num, n_position)
{
	m_data.Attach(basic->GetDisk(), n_block_num, n_position);
}
DiskBasicDirItemMSDOS::DiskBasicDirItemMSDOS(DiskBasic *basic, int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next, bool &n_unuse)
...
/// @param [in]  n_position   セクタ内のディレクトリエントリの位置
/// @param [out] n_next       次のセクタ
void DiskBasicDirItemMSDOS::SetDataPtr(int n_num, const DiskBasicGroupItem *n_gitem, int n_block_num, int n_position, const int *n_next)
{
	DiskBasicDirItem::SetDataPtr(n_num, n_gitem, n_block_num, n_position, n_next);
...
bool DiskBasicDirItemMSDOS::Check(bool &last)
{
	...
	bool valid = true;
	// ファイルサイズが大きい
	if (CheckUsed(false) && wxUINT32_SWAP_ON_BE(m_data.Data()->msdos.file_size) > 0xfffffff) {
		valid = false;
	}
	return valid;
}
...
void DiskBasicDirItemMSDOS::GetUnitGroups(int fileunit_num, DiskBasicGroups &group_items)
{
	...
	// 12/16bit FAT
	wxUint32 group_system_code = type->GetGroupSystemCode();
	...
	while(working) {
		...
		if (next_group == group_num) {
			// 同じポジションならエラー
			rc = false;
		} else if (next_group >= group_system_code) {
			// 最終グループ
			working = false;
		...
		if (rc) {
			basic->GetNumsFromGroup(group_num, next_group, sector_size, remain, group_items);
//			m_file_size += (basic->GetSectorSize() * basic->GetSectorsPerGroup());
			remain -= (sector_size * basic->GetSectorsPerGroup());
			calc_groups++;
	...
	group_items.SetSize(calc_file_size > 0 ? calc_file_size : sector_size * basic->GetSectorsPerGroup() * calc_groups);
	group_items.SetSizePerGroup(sector_size * basic->GetSectorsPerGroup());
