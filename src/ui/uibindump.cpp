...
void UiDiskBinDumpFrame::SetDatas(int sec_pos, const wxUint8 *buf, size_t len)
{
	UiDiskBinDump *dump = GetDumpPanel();
	if (dump) dump->SetDatas(sec_pos, buf, len);
}

void UiDiskBinDumpFrame::AppendDatas(int sec_pos, const wxUint8 *buf, size_t len)
{
	UiDiskBinDump *dump = GetDumpPanel();
	if (dump) dump->AppendDatas(sec_pos, buf, len);
}
...
MyMemoryBuffer *UiDiskBinDump::AppendBuffer(int sec_pos, const wxUint8 *buf, size_t len)
{
	MyMemoryBuffer *p = new MyMemoryBuffer(len);
	p->SetSectorPos(sec_pos);
...
/// ダンプにデータを表示
void UiDiskBinDump::SetDatas(int sec_pos, const wxUint8 *buf, size_t len)
{
	ClearBuffer();
	SetDatasMain(AppendBuffer(sec_pos, buf, len));
}
...
/// ダンプにデータを表示（追記）
void UiDiskBinDump::AppendDatas(int sec_pos, const wxUint8 *buf, size_t len)
{
	AppendDatasMain(AppendBuffer(sec_pos, buf, len));
}
...
void UiDiskBinDump::SetDatasBinaryMain(const MyMemoryBuffer *buf, bool append)
{
	...
	txtHex->AppendText(wxString::Format(wxT("# Sector:%d\n"),buf->GetSectorPos()));	
