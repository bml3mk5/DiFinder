...
void IntNameBox::OnListItemSelected(wxListEvent& event)
{
	...
	if (frame->GetBinDumpFrame()) {
		...
		file_list->SetDumpData((int)sec_start, (int)sec_end);
	}
}
...
void IntNameBox::SetGroups(const DiskBasicGroups &vals)
{
	...
	if (lstGroups) {
		...
		for(size_t i=0; i < vals.Count(); i++) {
			...
			lstGroups->InsertItem(i, wxString::Format(wxT("%02x"), item->GetGroup()));
			int col = 1;
			lstGroups->SetItem(i, col++, wxString::Format(wxT("%d"), item->GetSectorStart()));
			lstGroups->SetItem(i, col++, wxString::Format(wxT("%d"), item->GetSectorEnd()));
