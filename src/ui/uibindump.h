...
public:
	...
	void SetDatas(int sec_pos, const wxUint8 *buf, size_t len);
	void AppendDatas(int sec_pos, const wxUint8 *buf, size_t len);
	void ClearDatas();
...
public:
	...
	int GetSectorPos() const { return sector_pos; }
	void SetSectorPos(int val) { sector_pos = val; }
};
...
public:
	...
	void ClearBuffer();
	MyMemoryBuffer *AppendBuffer(int sec_pos, const wxUint8 *buf, size_t len);
