...
#pragma pack(1)
/// HDI形式ヘッダ
typedef struct st_hdi_dsk_header {
...
//
//
DiskHDIParser::DiskHDIParser(DiskImageFile *file, short mod_flags, DiskResult *result)
	: DiskPlainParser(file, mod_flags, result)
{
}

DiskHDIParser::~DiskHDIParser()
{
}
...
/// @retval -1 エラーあり
/// @retval  1 警告あり
int DiskHDIParser::Parse(wxInputStream &istream, const DiskParam *disk_param, const BootParam *boot_param)
{
	if (!disk_param) {
...
int DiskHDIParser::Check(wxInputStream &istream, const DiskTypeHints *disk_hints, const DiskParam *disk_param, DiskParamPtrs &disk_params, DiskParam &manual_param)
{
	istream.SeekI(0);
