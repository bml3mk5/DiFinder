...
wxUint32 DiskD88Creator::CreateSector(int track_number, int side_number, int sector_number, int sector_size, int sectors_per_track, bool single_density, DiskD88Track *track)
{
	...
	DiskD88Sector *sector = new DiskD88Sector(sector_number, track_number, side_number, sector_number, -1, sector_size, sectors_per_track, single_density);
	track->Add(sector);
...
//	int *sector_nums = new int[sector_max + 1];
	...
	if (!track->CalcSectorNumbersForInterleave(param->GetInterleave(), sector_max, sector_nums, param->GetSectorNumberBaseOnDisk())) {
...
{
	DiskD88Disk *disk = new DiskD88Disk(file, disk_number, *param, diskname, write_protect);
