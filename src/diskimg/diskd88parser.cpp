...
wxUint32 DiskD88Parser::ParseSector(wxInputStream &istream, int disk_number, int track_number, int sector_nums, int sector_size, DiskImageTrack *track)
{
	...
	size_t header_size = istream.Read((void *)sector_header.Get(), sector_header.GetSize()).LastRead();
			
	// track number is same ?
	if (sector_header.IDC() != track_number) {
		result->SetWarn(DiskResult::ERRV_ID_TRACK, disk_number, track_number, sector_header.IDC(), sector_header.IDH(), sector_header.IDR());
	}
	// side number is valid ?
	if (sector_header.IDH() > 1) {
		result->SetWarn(DiskResult::ERRV_ID_SIDE, disk_number, track_number, sector_header.IDC(), sector_header.IDH(), sector_header.IDR());
	}
	int sector_number = sector_header.IDR();
	// sector number is valid ?
	if (sector_number <= 0) {
		result->SetWarn(DiskResult::ERRV_ID_SECTOR, disk_number, track_number, sector_header.IDC(), sector_header.IDH(), sector_header.IDR(), sector_nums);
	}
	// invalid sector size
	if (sector_header.GetBufferSize() > 2048) {
		result->SetWarn(DiskResult::ERRV_SECTOR_SIZE_SECTOR, disk_number, sector_header.IDC(), sector_header.IDH(), sector_header.IDR(), sector_header.IDN(), sector_header.GetBufferSize());
	} else if (sector_header.GetBufferSize() == 0) {
		result->SetWarn(DiskResult::ERRV_SECTOR_SIZE_SECTOR, disk_number, sector_header.IDC(), sector_header.IDH(), sector_header.IDR(), sector_header.IDN(), sector_header.GetBufferSize());
	}

	// 追加
	size_t data_size = sector_header.GetBufferSize();
...
wxUint32 DiskD88Parser::ParseDisk(wxInputStream &istream, size_t start_pos, int disk_number)
{
	...
	do {
		...
		wxUint32 header_size = (wxUint32)istream.Read((void *)disk_header.Get(), disk_header.GetSize()).LastRead();
		size = header_size;

		// EOF(0x1a)ならスキップ
		wxByte *p = (wxByte *)disk_header.Get();
		bool all_eot = true;
