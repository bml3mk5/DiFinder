...
/// @brief ブートストラップ 名前リストを保存 
class BootParamName : public DiskParamName
{
public:
	BootParamName();
	BootParamName(const wxString &name, int flags);
};

//////////////////////////////////////////////////////////////////////

/// @class BootParamNames
///
/// @brief BootParamName のリスト
WX_DECLARE_OBJARRAY(BootParamName, BootParamNames);
...
//	bool IsReversible() const { return reversible; }
	...
	/// @brief 説明を返す
	const wxString &GetDiskParamDescription() const { return description; }
	/// @brief ディスクパラメータを含めた説明文を返す
	wxString GetDiskParamDetails() const;
};
