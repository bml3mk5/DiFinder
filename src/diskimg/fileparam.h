...
private:
	...
	bool			m_writable;		///< 書き込み可能か
	wxString		m_description;	///< 説明

public:
	FileFormat();
	FileFormat(const wxString &name, bool writable, const wxString &desc);
	~FileFormat() {}
...
private:
	wxString m_ext;
	const wxString *p_desc;
public:
	ExtensionItem();
	ExtensionItem(const wxString &ext, const wxString &desc);
	~ExtensionItem() {}
	const wxString &GetExt() const { return m_ext; }
	const wxString *GetDesc() const { return p_desc; }
};
...
///
/// @brief ExtensionItem のリスト
WX_DECLARE_OBJARRAY(ExtensionItem, ExtensionItems);
