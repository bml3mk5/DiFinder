...
public:
	DiskD88SectorHeader();
	DiskD88SectorHeader(d88_sector_header_t &src);
	~DiskD88SectorHeader();
	bool Copy(DiskImageSectorHeader &src);
	size_t GetSize() const;
	void CopyTo(d88_sector_header_t &dst);
	d88_sector_header_t *Get();
	wxUint8 IDC() const;
	wxUint8 IDH() const;
	wxUint8 IDR() const;
	wxUint8 IDN() const;
	void IDC(wxUint8 val);
	void IDH(wxUint8 val);
	void IDR(wxUint8 val);
	void IDN(wxUint8 val);
	wxUint16 GetNumberOfSectors() const;
	void SetNumberOfSectors(wxUint16 val);
	wxUint16 GetBufferSize() const;
	void SetBufferSize(wxUint16 val);
};
...
public:
	DiskD88Sector(int n_num, DiskImageSectorHeader &n_header, wxUint8 *n_data);
	DiskD88Sector(int n_num, int id_c, int id_h, int id_r, int id_n, int buffer_size, int number_of_sector, bool single_density = false, int status = 0);
	~DiskD88Sector();
	...
	/// ヘッダをコピー
	bool	CopyHeaderTo(DiskImageSectorHeader &dst);
...
public:
	...
	/// インスタンス作成
	DiskImageSector *NewImageSector(int n_num, DiskImageSectorHeader &n_header, wxUint8 *n_data);
	/// インスタンス作成
	DiskImageSector *NewImageSector(int n_num, int id_c, int id_h, int id_r, int id_n, int buffer_size, int number_of_sector, bool single_density = false, int status = 0);

	/// セクタを追加する
	size_t	Add(DiskImageSector *newsec);
	/// トラック内のセクタデータを置き換える
	int		Replace(DiskImageTrack *src_track); 
	/// トラックに新規セクタを追加する
	...
	/// セクタリストを返す
	DiskImageSectors *GetSectors() const { return sectors; }
	/// セクタ数を返す
	int		GetSectorsPerTrack() const;
	/// 指定セクタ番号のセクタを返す
	DiskImageSector *GetSector(int sector_number);
	/// 指定位置のセクタを返す
	DiskImageSector *GetSectorByIndex(int pos);
	...
	bool CalcSectorNumbersForInterleave(int interleave, size_t sectors_count, wxArrayInt &sector_nums, int sector_offset = 0);
};
...
public:
	DiskD88DiskHeader();
	DiskD88DiskHeader(d88_header_t &src);
	~DiskD88DiskHeader();
	bool Copy(DiskImageDiskHeader &src);
	size_t GetSize() const;
	void CopyTo(d88_header_t &dst);
	d88_header_t *Get();
	wxUint8 GetName(int pos) const;
	wxUint32 GetDiskSize() const;
	void SetDiskSize(wxUint32 val);
	wxUint32 GetOffset(int pos) const;
	void SetOffset(int pos, wxUint32 val);
	void ClearOffsets();
...
public:
	...
	DiskD88Disk(DiskD88File *file, int n_num, const DiskParam &n_param, const wxString &n_name, bool n_write_protect);
	DiskD88Disk(DiskD88File *file, int n_num, DiskImageDiskHeader &n_header);
	~DiskD88Disk();

	// インスタンス作成
	DiskImageTrack *NewImageTrack(int newtrknum, int newsidnum, int newoffpos, int newinterleave);

	/// ディスクにトラックを追加
	size_t	Add(DiskImageTrack *newtrk);
	/// ディスクの内容を置き換える
	int		Replace(int side_number, DiskImageDisk *src_disk, int src_side_number);
	/// ディスクにトラックを追加
...
//	DiskD88File   *GetFile() const { return parent; }
	/// トラックリストを返す
	DiskImageTracks *GetTracks() const { return tracks; }
	/// 指定トラックを返す
	DiskImageTrack  *GetTrack(int track_number, int side_number);
	/// 指定トラックを返す
	DiskImageTrack  *GetTrack(int index);
	/// 指定オフセット値からトラックを返す
	DiskImageTrack  *GetTrackByOffset(wxUint32 offset);
	/// 指定セクタを返す
	DiskImageSector *GetSector(int track_number, int side_number, int sector_number);
...
public:
	...
	/// インスタンス作成
	DiskImageDisk *NewImageDisk(int disk_number);
	/// インスタンス作成
...
//	};
	...
	size_t Add(DiskImageDisk *newdsk, short mod_flags);
	void Clear();
	size_t Count() const;
	bool Delete(size_t idx);

	DiskImageDisks *GetDisks() { return disks; }
...
public:
	...
	/// 置換元のディスクを解析
	int ParseForReplace(int disk_number, int side_number, const wxString &filepath, const wxString &file_format, const DiskParam &param_hint, DiskImageFile &src_file, DiskImageDisk* &tag_disk);
	/// ファイルでディスクを置換
	int ReplaceDisk(int disk_number, int side_number, DiskImageDisk *src_disk, int src_side_number, DiskImageDisk *tag_disk);
	...
	/// ディスクファイルを返す
	DiskImageFile		*GetFile() { return file; }
	/// ディスクファイルを返す
	const DiskImageFile	*GetFile() const { return file; }
	/// ディスク枚数
	size_t CountDisks() const;
	/// ディスク一覧を返す
	DiskImageDisks *GetDisks();
	/// 指定した位置のディスクを返す
	DiskImageDisk		*GetDisk(size_t index);
	/// 指定した位置のディスクを返す
	const DiskImageDisk	*GetDisk(size_t index) const;
	/// 指定した位置のディスクのタイプ
