...
// DISK BASIC 名前リストを保存
//
BootParamName::BootParamName()
	: DiskParamName()
{
}
BootParamName::BootParamName(const wxString &name, int flags)
...
#include <wx/arrimpl.cpp>
WX_DEFINE_OBJARRAY(BootParamNames);
...
{
	bool match = (sides_per_disk == param.sides_per_disk)
		&& (tracks_per_side == param.tracks_per_side)
		&& (sectors_per_track == param.sectors_per_track)
		&& (sector_size == param.sector_size)
		&& (interleave == param.interleave)
...
/// @param[in] flags     フラグ
/// @return 名前
const BootParamName *DiskParam::FindBootType(const wxString &type_name, int flags) const
{
	const BootParamName *match = NULL;
	for(size_t i=0; i<boot_types.Count(); i++) {
		const BootParamName *item = &boot_types.Item(i);
...
int DiskParam::MatchBootType(const wxArrayString &type_names, int flags) const
{
	int match = wxNOT_FOUND;
	for(size_t i=0; i<type_names.Count(); i++) {
		if (FindBootType(type_names.Item(i), flags)) {
			match = (int)i;
			break;
		}
	}
...
/// @return 文字列
wxString DiskParam::GetDiskParamDetails() const
{
	wxString str;
	...
	while (item) {
		if (item->GetName() == "DiskType") {
			...
			while (itemnode) {
				...
				} else if (itemnode->GetName() == "BootTypes") {
					BootParamNames boot_types;
					if (!LoadBootTypes(itemnode, boot_types, errmsgs)) {
						return false;
					}
					p.SetBootTypes(boot_types);
