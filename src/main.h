...
/// メインFrame
class UiDiskFrame: public wxFrame
{
private:
	// gui
	MyMenu *menuFile;
...
public:
	...
	/// ステータスメッセージ更新タイマー
	void OnTimerStatusTimer(wxTimerEvent& event);
	...
	void SetDataOnFile(DiskImageFile *file);
	...
	/// ディレクトリをアサイン
	bool AssignDirectory(DiskImageDisk *disk, int side_num, DiskBasicDirItem *dir_item);
	/// ディレクトリを移動
	bool ChangeDirectory(DiskImageDisk *disk, int side_num, DiskBasicDirItem *dir_item, bool refresh_list);
	/// ディレクトリを削除
	bool DeleteDirectory(DiskImageDisk *disk, int side_num, DiskBasicDirItem *dir_item);
	//@}
	...
	void RefreshAllDirectoryNodesOnDiskList(DiskImageDisk *disk, int side_number);
	...
	void SetDiskAttrData(DiskImageFile *file);
	/// 右上パネルのディスク属性にデータを設定する
	...
	void SetRawPanelData(DiskImageFile *file);
	/// 右下パネルのRAWディスクパネルにデータを設定する
	void SetRawPanelData(DiskImageDisk *disk);
	...
	/// ダンプウィンドウにデータを設定する
	void SetBinDumpData(int sec_pos, const wxUint8 *buf, size_t len, const wxString &char_code, bool invert);
	/// ダンプウィンドウにデータを設定する
	void SetBinDumpData(int sec_pos, const wxUint8 *buf, size_t len);
	/// ダンプウィンドウにデータを追記する
	void AppendBinDumpData(int sec_pos, const wxUint8 *buf, size_t len, const wxString &char_code, bool invert);
	/// ダンプウィンドウにデータを追記する
	void AppendBinDumpData(int sec_pos, const wxUint8 *buf, size_t len);
	/// ダンプウィンドウをクリア
...
/// ファイルダイアログ
class UiDiskFileDialog: public wxFileDialog
{
public:
	UiDiskFileDialog(const wxString& message, const wxString& defaultDir = wxEmptyString, const wxString& defaultFile = wxEmptyString, const wxString& wildcard = wxFileSelectorDefaultWildcardStr, long style = wxFD_DEFAULT_STYLE);
